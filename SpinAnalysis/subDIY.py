...
parser.add_option("--globePDFs",dest="globePDFs",action="store_true",default=False)
...
for j in range(options.njobs):
  ...
  subline = './diySeparation %d %s/%s %d %d'%(options.toyspj,os.getcwd(),options.file,options.bcats,options.scats)
  if options.globePDFs: subline += ' --globePDFs'
  f.write('if ( %s )\n'%subline)
