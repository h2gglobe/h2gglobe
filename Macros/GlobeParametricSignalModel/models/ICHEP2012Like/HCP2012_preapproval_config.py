...
if year == 2011:
    ...
    #inputFile = "/afs/cern.ch/work/x/xquan/CMSSW_5_2_4/src/HiggsAnalysis/HiggsTo2photons/oct_h2gglobe/Macros/ResultScripts/xiaohang/CMS-HGG.root"
    inputFile = "/afs/cern.ch/work/m/mkenzie/private/h2g/latest_workspaces/CMS-HGG_massfacmva_hcpPreapproval.root"
    outputFile = "/afs/cern.ch/work/m/mkenzie/private/h2g/latest_workspaces/Sig-Mod_massfacmva_hcpPreapproval.root"
...
globalConfig = {
    ...
    # output filename
    "outputWorkspaceFname": outputFile,
...
if year == 2011:
    ...
    assert(numInclusiveCategories == 4)
    assert(numCategories == 9)
    
    tmp = {
        'cat0': { 'ggh': 3, 'vbf': 3, 'wzh': 3, 'tth': 3 },
        'cat1': { 'ggh': 3, 'vbf': 3, 'wzh': 3, 'tth': 3 },
        'cat2': { 'ggh': 3, 'vbf': 3, 'wzh': 3, 'tth': 3 },
        'cat3': { 'ggh': 3, 'vbf': 3, 'wzh': 3, 'tth': 3 },

        'cat4': { 'ggh': 2, 'vbf': 3, 'wzh': 1, 'tth': 2 },
        'cat5': { 'ggh': 2, 'vbf': 3, 'wzh': 2, 'tth': 2 },

        'cat6': { 'ggh': 1, 'vbf': 1, 'wzh': 3, 'tth': 3 },
        'cat7': { 'ggh': 1, 'vbf': 1, 'wzh': 3, 'tth': 3 },
        'cat8': { 'ggh': 1, 'vbf': 1, 'wzh': 3, 'tth': 3 },

        }
    tmp_wrong = {
       'cat0': { 'ggh': 1, 'vbf': 2, 'wzh': 1, 'tth': 1 },
       'cat1': { 'ggh': 2, 'vbf': 2, 'wzh': 2, 'tth': 1 },
       'cat2': { 'ggh': 2, 'vbf': 2, 'wzh': 2, 'tth': 1 },
       'cat3': { 'ggh': 2, 'vbf': 2, 'wzh': 2, 'tth': 1 },

       'cat4': { 'ggh': 1, 'vbf': 2, 'wzh': 1, 'tth': 1 },
       'cat5': { 'ggh': 1, 'vbf': 2, 'wzh': 1, 'tth': 1 },

       'cat6': { 'ggh': 1, 'vbf': 1, 'wzh': 1, 'tth': 1 },
       'cat7': { 'ggh': 1, 'vbf': 1, 'wzh': 2, 'tth': 1 },
       'cat8': { 'ggh': 1, 'vbf': 1, 'wzh': 2, 'tth': 1 },
    ...
    for cat,data in tmp_wrong.items():
        for proc, num in data.items():
            numGaussians[cat]['wrong'][proc] = num
...
smearings = {
    ...
    '8TeV': {catNamePrefix + '0': 0.008438,
             ...
             catNamePrefix + '6': 0.0117,
             catNamePrefix + '7': 0.0117,
             catNamePrefix + '8': 0.0117,
             },
          }
    ...
    for vertex in ('right', 'wrong'):
        for proc in allProcs:
            for index in range(numGaussians[catname][vertex][proc]):
                index += 1
                fitParameterSettings[catname][vertex][proc]['sigma%d' % index]['initial'] = 1 + index
                fitParameterSettings[catname][vertex][proc]['sigma%d' % index]['max'] = 20
                if vertex == 'right': fitParameterSettings[catname][vertex][proc]['sigma%d' % index]['min'] = 0.7
                else : fitParameterSettings[catname][vertex][proc]['sigma%d' % index]['min'] = 1.
                number = index - 1
                if (number % 2 == 0):
                    sign = -1
                else:
                ...
                fitParameterSettings[catname][vertex][proc]['dm%d' % index]['initial'] = 0.005 * sign
                if numGaussians[catname][vertex][proc] == 1 : 
                  fitParameterSettings[catname][vertex][proc]['dm%d' % index]['min'] = -10
                  fitParameterSettings[catname][vertex][proc]['dm%d' % index]['max'] = +10
                elif numGaussians[catname][vertex][proc] == 2 : 
                  if index ==1: 
                    fitParameterSettings[catname][vertex][proc]['dm%d' % index]['min'] = -10
                  ...
                  if index == 1: 
                    fitParameterSettings[catname][vertex][proc]['dm%d' % index]['min'] = -10.
                    fitParameterSettings[catname][vertex][proc]['dm%d' % index]['max'] = -1.
                  elif index ==2: 
                    fitParameterSettings[catname][vertex][proc]['dm%d' % index]['min'] = -1.
                    fitParameterSettings[catname][vertex][proc]['dm%d' % index]['max'] = 1.
                  else : 
                    fitParameterSettings[catname][vertex][proc]['dm%d' % index]['min'] = 1.
                    fitParameterSettings[catname][vertex][proc]['dm%d' % index]['max'] = 10.
