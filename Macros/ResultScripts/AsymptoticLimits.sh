...
for FILE in `/bin/ls higgsCombineTest.Asymptotic.*.root`; do
	TARGETFILE=`echo "$FILE" | sed 's|higgsCombineTest.Asymptotic.mH\(1[0-9][0-9][\.5]*\)\.[0-9-][0-9-]*.root|higgsCombineTest.Asymptotic.mH\1.root|' | sed 's|mH\(1[0-9][0-9]\).root|mH\1.0.root|'`
	mv $FILE $TARGETFILE
done
cd ..
