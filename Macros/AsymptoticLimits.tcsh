...
foreach i (`seq 110 0.5 150`)
	...
	while ($threads >= $maxthreads)
		echo "The Computer is Tired: Resting for 5 seconds"
		sleep 5
		set threads = `ps | grep combine | wc | awk '{print $1}'`
	end
...
foreach i (`/bin/ls higgsCombineTest.Asymptotic.*.root`)
	set targetfile = `echo "$i" | sed 's|higgsCombineTest.Asymptotic.mH\(1[0-9][0-9][\.5]*\)\.[0-9-][0-9-]*.root|higgsCombineTest.Asymptotic.mH\1.root|' | sed 's|mH\(1[0-9][0-9]\).root|mH\1.0.root|'`
	mv $i $targetfile
end
cd ..
