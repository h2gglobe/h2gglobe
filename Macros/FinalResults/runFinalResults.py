...
# make plots
  ...
  for card, suffix, line in config:
    ...
    if options.parametric: os.system("python limitPlotter.py -M ProfileLikelihood --pval -p %s/%s/ExpProfileLikelihood_m125 -o %s/%s/exppval_m125.root"%(result_dir,suffix,result_dir,suffix))
    pval_command = "python limitPlotter.py -M ProfileLikelihood --pval --addline=\"%s/%s/exppval.root:1:2:Expected 1xSM\" --addtxt=\"%s\""%(result_dir,suffix,line)
    ...
    else: pval_command += " -p %s/%s/ExpProfileLikelihood -e -o %s/%s/exppval.root"%(result_dir,suffix,result_dir,suffix)
