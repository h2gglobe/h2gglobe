...
for m in {110..150}; do 
  ...
  combine mva-datacard_$2_$m.0.txt -M MaxLikelihoodFit -D data_$2 -m $m --minimizerStrategy=2 --rMin=-3. --rMax=3. 
  combine mva-datacard_$2_$m.0.txt -M ProfileLikelihood -m $m --rMin=0. --rMax=25 --signif --pvalue 
done
...
for m in {110..149}; do 
  ...
  combine mva-datacard_$2_$m.5.txt -M MaxLikelihoodFit -D data_$2 -m $m.5 --minimizerStrategy=2 --rMin=-3. --rMax=3. 
  combine mva-datacard_$2_$m.5.txt -M ProfileLikelihood -m $m.5 --rMin=0. --rMax=25 --signif --pvalue 
done
...
for m in {110..150}; do 
  ...
  echo -------------------------------
  combine mva-datacard_$2_$m.0.txt -M ProfileLikelihood -m $m --rMin=0. --rMax=25 --signif --pvalue -t -1 --expectSignal=1 
...
for m in {110..149}; do 
  ...
  echo Running $2 $1 limit for mass $m.5 
  echo -------------------------------
  combine mva-datacard_$2_$m.5.txt -M ProfileLikelihood -m $m.5 --rMin=0. --rMax=25 --signif --pvalue -t -1 --expectSignal=1 
  mv higgsCombineTest.ProfileLikelihood.mH$m.5.root ExpProfileLikelihood/higgsCombineTest.ProfileLikelihood.mH$m.5.root
done

methods=( Asymptotic AsymptoticNew MaxLikelihoodFit ProfileLikelihood ExpProfileLikelihood )
for meth in ${methods[@]}; 
  do hadd -f $meth/$meth.root $meth/higgsCombineTest.*; 
done
