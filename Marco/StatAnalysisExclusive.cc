...
void StatAnalysisExclusive::Analysis(LoopAll& l, Int_t jentry) 
{
	...
	t_dmodz = l.getDmOverDz(l.dipho_leadind[diphoton_id_jim], l.dipho_subleadind[diphoton_id_jim], &smeared_pho_energy[0]);
	float z_gg = ((TVector3*)(l.vtx_std_xyz->At(l.dipho_vtxind[diphoton_id_jim])))->Z();
	t_sigma_mz = t_dmodz*(sqrt(pow(double(l.bs_sigmaZ), 2) + pow(double(z_gg), 2)));
	t_bsZ = l.bs_sigmaZ;
...
void StatAnalysisExclusive::HggBookOptree() {
  ...
  // NEW branches
  // FIXME MATTEO
  optree->Branch("pvtx", &t_pvtx, "pvtx/F", 20000000);
  optree->Branch("dmodz", &t_dmodz, "dmodz/F", 20000000);
  optree->Branch("sigma_mz", &t_sigma_mz, "sigma_mz/F", 20000000);
  optree->Branch("bsZ", &t_bsZ, "bsZ/F", 20000000);
...
//MARCO FIXED
  ...
  float dmom=0.5*pow(pow(double(ll->pho_regr_energyerr[leadind]/ll->pho_regr_energy[leadind]), 2)+pow(double(ll->pho_regr_energyerr[subleadind]/ll->pho_regr_energy[subleadind]), 2), 0.5);
...
void StatAnalysisExclusive::SetBDT() {
  ...
  std::cout<<"SetBDT"<<std::endl;
  tmvaReader->BookMVA("Gradient", "/home/users/matteo/mh_110_135_Gradient.weights.xml");
  std::cout<<"SetBDT"<<std::endl;
  ...
  tmvaReader1->BookMVA("Category_Gradient", "/home/users/matteo/mh_110_135_Category_Gradient.weights.xml");
  */
...
if(jentry%1000==1&&itype>0&&itype%1000==1) cout<<jentry<<"  "<<itype<<"  mass="<<mass<<" out  higgs_genpt= NOT THERE  diphopt="<<diphotonp4.Pt()<<"  t_w="<<t_w<<endl;
  ...
  t_mass = mass;
  t_dmom0 = 0.5*pow(pow(double(ll->pho_regr_energyerr[leadind]/ll->pho_regr_energy[leadind]), 2)+pow(double(ll->pho_regr_energyerr[subleadind]/ll->pho_regr_energy[subleadind]), 2), 0.5);
  ...
  double errfrac=0.5*pow(pow(double(ll->pho_regr_energyerr[leadind]/ll->pho_regr_energy[leadind]), 2)+pow(double(leadErr), 2)+pow(double(ll->pho_regr_energyerr[subleadind]/ll->pho_regr_energy[subleadind]), 2)+pow(double(subleadErr), 2), 0.5);
  t_dmom = errfrac;
