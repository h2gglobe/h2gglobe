...
void StatAnalysis::Analysis(LoopAll& l, Int_t jentry) 
{
	...
	t_pvtx = vtxAna_.vertexProbability((*l.vtx_std_evt_mva)[l.dipho_vtxind[diphoton_id_jim]]);
        ...
        t_bsZ = l.bs_sigmaZ;
	cout<<"MPMP 222"<<endl;
	t_leadphoidmitmva = l.photonIDMVA(l.dipho_leadind[diphoton_id_jim], l.dipho_vtxind[diphoton_id_jim], lead_p4, diphoton.M(), "MIT");
	cout<<"MPMP 223"<<endl;
	t_subleadphoidmitmva = l.photonIDMVA(l.dipho_subleadind[diphoton_id_jim], l.dipho_vtxind[diphoton_id_jim], sublead_p4, diphoton.M(), "MIT");
	cout<<"MPMP 224"<<endl;
	...
	t_diphomitmva = l.diphotonMVA(l.dipho_leadind[diphoton_id_jim], l.dipho_subleadind[diphoton_id_jim], l.dipho_vtxind[diphoton_id_jim], t_pvtx, lead_p4, sublead_p4, t_sigmaMRV, t_sigmaMWV, sigmaMeonly, "MIT");
	cout<<"MPMP 227"<<endl;
	...
	if(!l.GetCutValue("optree",0)) {
	  ...
	  SetOutputNtupleVariables(jentry, itypepass, l.dipho_leadind[diphotonVBF_id], l.dipho_subleadind[diphotonVBF_id], l.dipho_vtxind[diphotonVBF_id], diphoton.M(), &lead_p4, &sublead_p4, evweight, pileupWeight, jet1, jet2);
...
#ifdef NewFeatures
void StatAnalysis::HggBookOptree() {
...
void StatAnalysis::setDiphoCuts() {
...
void StatAnalysis::setPhotonCuts6() {
...
void StatAnalysis::setPhotonCuts6pf() {
...
void StatAnalysis::setPhotonCuts4() {
...
int StatAnalysis::photonCutLevel4(float r9, float eta, float et, float isosum, float isosumbad, float tkiso, float sieie, float hoe, float drtotk) {
...
int StatAnalysis::photonCutLevel6(float r9, float eta, float et, float isosum, float isosumbad, float tkiso, float sieie, float hoe, float drtotk) {
...
int StatAnalysis::photonCutLevel6pf(float r9, float eta, float et, float isosum, float isosumbad, float chargediso, float neutraliso, float sieie, float hoe, float drtotk) {
  //  Returns the photon cut level for 6 categories from 0 (passing no cut level) to 11 (passing all cut levels).
  //  I think this should move to General Functions so that it can be used for photon ID in general, but also GA and CMSSW.
...
int StatAnalysis::diphoCutLevel(int leadind, int subleadind, int vtxind) {
...
int StatAnalysis::diphoSubCategory(int nSubCat, int c4, int diphoCutLev) {
...
float StatAnalysis::diphoSubCategory(int c4, int diphoCutLev) {
...
Float_t StatAnalysis::BDT(Int_t jentry, Int_t iPhoton, Int_t vtx) {
...
Float_t StatAnalysis::BDT_categorized(Int_t jentry, Int_t iPhoton, Int_t vtx, float isLead) {
...
Float_t StatAnalysis::BDT_ptom(Int_t jentry, Int_t iPhoton, Int_t vtx, float mass) {
...
Float_t StatAnalysis::BDT_ptom2(Int_t jentry, Int_t iPhoton, Int_t vtx, float mass) {
...
Float_t StatAnalysis::BDT_dipho(Int_t jentry, Int_t isl, Int_t il, float lmva, float slmva, float diphopt, float mass, float dmom) {
...
Float_t StatAnalysis::BDT_dipho2(Int_t jentry, Int_t isl, Int_t il, float lmva, float slmva, float diphopt, float mass) {
...
void StatAnalysis::SetOutputNtupleVariables(int jentry, int itype, int leadind, int subleadind, int vtxind, float mass, TLorentzVector *leadp4, TLorentzVector *subleadp4, float evweight, float pileupWeight, TLorentzVector * jet1, TLorentzVector * jet2) {
...
if(jentry%1000==1&&itype>0&&itype%1000==1) cout<<jentry<<"  "<<itype<<"  mass="<<mass<<" out  higgs_genpt= NOT THERE  diphopt="<<diphotonp4.Pt()<<"  t_w="<<t_w<<endl;
  ...
  if(!ll->GetCutValue("optree",0)) return;
...
//MARCO FIX CHECK
  ...
  if(ll->GetCutValue("bdt",0)) {
    ...
    t_leadci6cpfmvaptom = 0; //MARCOFIX BDT_ptom(jentry, leadind,vtxind, mass);
    t_leadci6cpfmvaptom2 = 99;//BDT_ptom2(jentry, leadind,vtxind, mass);
    t_diphomva = 0; //MARCOFIX BDT_dipho(jentry, leadind, subleadind, t_leadci6cpfmvaptom, t_subleadci6cpfmvaptom, t_diphopt, mass, t_dmom);
...
//MARCO FIX CHECK
  ...
  cout<<"MPMP here 100"<<endl;

  if(MPDEBUGCOPY) cout << "SetOutputNtupleVariables END"<<endl;
}

std::vector<Float_t> StatAnalysis::arrayToVector(size_t length, const Float_t *data) {
  std::vector<float> retval;
  std::copy (data, data + length,std::back_inserter ( retval ) );
  return retval;
}
float StatAnalysis::GetSmearSigma(float eta, float r9, int epoch){
...
Int_t StatAnalysis::GenIndexHgg(TLorentzVector *p_p4) {
...
Int_t StatAnalysis::itype_jim(int itype) {
...
// ---------------------------------------------------------------------------------------------------------------------------------------------
std::pair<int,int> StatAnalysis::DiphotonCiCSelectionIndicesJim( phoCiCIDLevel LEADCUTLEVEL, phoCiCIDLevel SUBLEADCUTLEVEL, Float_t leadPtMin, Float_t subleadPtMin, int ncat, int vtxind, bool applyPtoverM) {
...
// ---------------------------------------------------------------------------------------------------------------------------------------------
int   StatAnalysis::PhotonCiCSelectionLevelJim( int photon_index, int ncat, int vtxind, int doSublead, int diphoind, int print) {
...
// ---------------------------------------------------------------------------------------------------------------------------------------------
int   StatAnalysis::PhotonCiCpfSelectionLevelJim( int photon_index, int ncat, int vtxind, int doSublead, int diphoind, int print) {
  //  This routine picks out the correct values of the isolation etc. so that the photon id can be applied.
