...
void PdfWeightSmearer::readFile(std::string uId, std::string dId ){
  ...
  TH1* temp = (TH2F*) thePdfWeightFile_->Get("GF_cent");
...
double PdfWeightSmearer::getWeight( const TLorentzVector & p4, const int nPu, const int & genMassPoint, float syst_shift) const
{
  float gPT = p4.Pt();
  float gY  = fabs(p4.Rapidity());

  int    varId     = syst_shift > 0 ?   1 : 2;
  double nominal   = getPdfWeight( genMassPoint, 0, gPT, gY );
  double variation = getPdfWeight( genMassPoint, varId, gPT, gY );
