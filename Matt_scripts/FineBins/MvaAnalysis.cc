...
void MvaAnalysis::Term(LoopAll& l) 
{
    ...
    if (doTraining){
        ...
        // loop hypothesis  
        for (double mass=120.0; mass<130.1; mass+=0.1){
            ...
            */
            ...
            if (fabs(mass-120)<0.001 || fabs(mass-125)<0.001 || fabs(mass-130)<0.001){
        ...
        for (int i=3; i<6; i++){
            ...
            for (double mass=mass_h_low; mass<mass_h_high; mass+=0.1){

                double mass_hypothesis = masses[i]+mass;
                if (mass_hypothesis < 120 || mass_hypothesis > 130) continue;
...
void MvaAnalysis::Init(LoopAll& l) 
{
    ...
    */
    ...
    if (doTraining){
        ...
        for (double sig=120.0;sig<=130;sig+=5.0){  // We are ignoring mass 105 for now
        ...
        if (doTraining ){//
            ...
            // ---- Now deal with background MC and data
                ...
                for (int i = 1; i<nMasses;i++){ //ignoring mass 105 for now
                    ...
                    for (double mass_h=mass_h_low; mass_h<mass_h_high; mass_h+=0.1){
