...
for type in Asymptotic ProfileLikelihood MaxLikelihoodFit; do
  mkdir -p $path/combineResults_v2/$2/$type 
  mv higgsCombineTest.${type}*.root $path/combineResults_v2/$2/$type
  for m in {110..150}; do
    mv $path/combineResults_v2/$2/$type/higgsCombineTest.$type.mH$m.root $path/combineResults_v2/$2/$type/higgsCombineTest.$type.mH$m.0.root
  done
done
