void eff_pho(bool isSignal=true, bool saveToWeb=false) {
  ...
  TFile *f = TFile::Open("../AnalysisScripts/mvaWP_"+sigBkg_str+"/combined/histograms_CMS-HGG.root");
  ...
  leg->SetFillColor(10);
  leg->SetTextSize(.035);
  leg->AddEntry(hist_eta_sel[0][0],"category 0 (EB, R_{9}>0.94)","LP");
  leg->AddEntry(hist_eta_sel[1][0],"category 1 (EB, R_{9}<0.94)","LP");
  leg->AddEntry(hist_eta_sel[2][0],"category 2 (EE, R_{9}>0.94)","LP");
  leg->AddEntry(hist_eta_sel[3][0],"category 3 (EE, R_{9}<0.94)","LP");
