void massPlot(TString category="0", TString var="all_mass", bool breakdown=false) {
  ...
  TFile *f = TFile::Open("histograms_CMS-HGG_massfacmva.root");
  TFile *f_qcd40 = TFile::Open("histograms_CMS-HGG_massfacmva_qcd40only.root");
  ...
  hist_mass_qcd_ff = (TH1*)(f_qcd40->Get(var+"_cat"+category+"_QCD40FF"))->Clone();
  hist_mass_qcd_ff->Scale(36./35.);
  hist_mass_qcd30_ff = (TH1*)(f->Get(var+"_cat"+category+"_QCD30FF"))->Clone();
  hist_mass_qcd_ff->Add(hist_mass_qcd30_ff);

  hist_mass_qcd_pf = (TH1*)(f_qcd40->Get(var+"_cat"+category+"_QCD40PF"))->Clone();
  hist_mass_qcd_pf->Scale(101./95.);
...
//   cout << hist_mass_dy->Integral() << endl;
  ...
  if (var=="all_mass") {
    //hist_mass_bkg_stack->SetMaximum(5000.);
    //hist_mass_bkg_stack->SetMaximum(2500.);
    hist_mass_bkg_stack->SetMaximum(1250.);
  ...
  hist_mass_bkg_stack->GetYaxis()->SetTitle("Events / 1 GeV");
  ...
  txt->SetTextAlign(12);
  txt->DrawLatex(0.31,0.85,"#scale[0.8]{#splitline{CMS preliminary}{#sqrt{s} = 8 TeV L = 5.3 fb^{-1}}}");
